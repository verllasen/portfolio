...
export default function Dashboard() {
  const { 
    ...
    addGroup, addChannel, addFriend, removeFriend, acceptFriend, leaveGroup,
  ...
  useEffect(() => {
    if (user) {
      ...
      fetch(`${API_URL}/groups/${user.id}`)
        ...
        .then(data => {
          if (Array.isArray(data)) {
            useStore.getState().setGroups(data);
            const allChannels = data.flatMap(g => g.channels || []);
            useStore.getState().setChannels(allChannels);
          }
        })
      ...
      fetch(`${API_URL}/friends/${user.id}`)
        ...
        .then(data => {
          if (Array.isArray(data)) {
            // We need a setFriends method in store, let's just use addFriend in a loop or add setFriends
            data.forEach(f => addFriend(f));
          }
        })
    ...
    const handleCallMade = (data: any) => {
      if (localStorage.getItem('callSound') !== 'false') playCallSound();
      if (localStorage.getItem('sysNotifications') !== 'false' && 'Notification' in window && Notification.permission === 'granted') {
    ...
    const handleFriendRequest = (data: any) => {
      addFriend({ id: data.id, name: data.name, status: 'pending_incoming' });
      if (localStorage.getItem('sysNotifications') !== 'false' && 'Notification' in window && Notification.permission === 'granted') {
        new Notification(`Новая заявка в друзья от ${data.name}`);
      }
    };
    ...
    const handleFriendAccepted = (data: any) => {
      ...
      if (localStorage.getItem('sysNotifications') !== 'false' && 'Notification' in window && Notification.permission === 'granted') {
        new Notification(`${data.name} принял(а) заявку в друзья`);
      }
    };
  ...
  const handleAddFriend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (friendIdInput.trim() && user) {
      try {
        const API_URL = import.meta.env.PROD ? 'https://friendly-server-w6zh.onrender.com/api' : 'http://localhost:3001/api';
        const res = await fetch(`${API_URL}/friends/request`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fromUserId: user.id, toUserId: friendIdInput.trim() })
        });
    ...
    if (user) {
      try {
        ...
        const res = await fetch(`${API_URL}/friends/accept`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ fromUserId: friendId, toUserId: user.id })
        });
    ...
    if (groupNameInput.trim() && user) {
      try {
        ...
        const res = await fetch(`${API_URL}/groups`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: groupNameInput.trim(), ownerId: user.id })
        });
        const newGroup = await res.json();
        addGroup(newGroup);
        if (newGroup.channels && newGroup.channels.length > 0) {
          newGroup.channels.forEach((c: any) => addChannel(c));
          setActiveRoom(newGroup.channels[0].id);
        }
        setGroupNameInput('');
    ...
    if (joinGroupInput.trim() && user) {
      try {
        ...
        const res = await fetch(`${API_URL}/groups/join`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ inviteCode: joinGroupInput.trim(), userId: user.id })
        });
        ...
        if (res.ok) {
          ...
          if (group.channels && group.channels.length > 0) {
            group.channels.forEach((c: any) => addChannel(c));
          }
          setJoinGroupInput('');
  ...
  const handleCreateChannel = async (e: React.FormEvent) => {
    ...
    if (channelNameInput.trim() && activeGroupId) {
      try {
        const API_URL = import.meta.env.PROD ? 'https://friendly-server-w6zh.onrender.com/api' : 'http://localhost:3001/api';
        const res = await fetch(`${API_URL}/groups/${activeGroupId}/channels`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: channelNameInput.trim(), type: channelTypeInput })
        });
                          ...
                          <div key={f.id} className="p-3 rounded-xl flex items-center justify-between mb-1 bg-[#111]">
                            ...
                            <div className="flex gap-1">
                              {f.status === 'pending_incoming' && (
                                <button onClick={() => handleAcceptFriend(f.id)} className="p-1.5 bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white rounded-md transition-colors">
                                  <UserCheck size={16} />
                                </button>
                              )}
...
function SettingsPanel({ user }: { user: any }) {
  ...
  const [chatSoundEnabled, setChatSoundEnabled] = useState(() => localStorage.getItem('chatSound') !== 'false');
  const [callSoundEnabled, setCallSoundEnabled] = useState(() => localStorage.getItem('callSound') !== 'false');
  const [sysNotificationsEnabled, setSysNotificationsEnabled] = useState(() => localStorage.getItem('sysNotifications') !== 'false');

  const toggleChatSound = () => {
    const newVal = !chatSoundEnabled;
    setChatSoundEnabled(newVal);
    localStorage.setItem('chatSound', String(newVal));
  };
  
  const toggleCallSound = () => {
    const newVal = !callSoundEnabled;
    setCallSoundEnabled(newVal);
    localStorage.setItem('callSound', String(newVal));
  };

  const toggleSysNotifications = () => {
    const newVal = !sysNotificationsEnabled;
    setSysNotificationsEnabled(newVal);
    localStorage.setItem('sysNotifications', String(newVal));
          ...
          <div className="max-w-2xl">
            ...
            <div className="bg-[#111] border border-[#222] rounded-2xl p-6 space-y-6">
              <div className="flex items-center justify-between">
                ...
                <div onClick={toggleChatSound} className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${chatSoundEnabled ? 'bg-green-500' : 'bg-gray-600'}`}>
                  <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${chatSoundEnabled ? 'right-1' : 'left-1'}`}></div>
                </div>
              </div>
              <div className="flex items-center justify-between pt-6 border-t border-[#222]">
                ...
                <div onClick={toggleCallSound} className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${callSoundEnabled ? 'bg-green-500' : 'bg-gray-600'}`}>
                  <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${callSoundEnabled ? 'right-1' : 'left-1'}`}></div>
                </div>
              </div>
              <div className="flex items-center justify-between pt-6 border-t border-[#222]">
                <div>
                ...
                <div onClick={toggleSysNotifications} className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${sysNotificationsEnabled ? 'bg-green-500' : 'bg-gray-600'}`}>
                  <div className={`w-4 h-4 bg-white rounded-full absolute top-1 transition-all ${sysNotificationsEnabled ? 'right-1' : 'left-1'}`}></div>
                </div>
              </div>
            </div>
          </div>
