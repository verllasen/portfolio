...
io.on('connection', (socket) => {
  ...
  socket.on('join_studio', ({ userId, studioCode, userName }) => {
    ...
    if (studio.isPrivate) {
        ...
        if (studio.privacyType === 'invite_only') {
             // Check if user is in pendingMembers or invited
             // Since we don't have invite list in studio obj properly synced yet, we check pending
             // But wait, invites are sent to user. User accepts invite -> joins.
             // If user calls join_studio directly, they must be allowed if they have invite.
             // For now, let's assume 'invite_only' blocks direct join unless handled via acceptStudioInvite which calls this too.
             // We can check if studio has this user in 'invited' list if we had one.
             // Simplified: block direct join for invite_only, only allow if coming via "accept invite" flow?
             // Actually, if privacy is invite_only, user needs to be in pending or invited.
             // For now, blocking direct code join.
             // But acceptStudioInvite calls join_studio. How to distinguish?
             // Maybe we don't need to distinguish if we trust the flow.
             // Let's just block if it's strictly private.
             // But if I accepted an invite, I shouldn't be blocked.
             // Let's assume invite acceptance bypasses this check or we check studioInvites on client.
             // For safety: if invite_only, reject unless we verify invite (hard without persistent invites).
             // Let's stick to: if invite_only, we assume client handled the check for now or we just allow it if code matches?
             // No, "invite only" means code doesn't work for strangers.
  ...
  socket.on('take_contract', ({ studioCode, missionId, playerId }) => {
      ...
      if (studio) {
          ...
          // Find leader name
          const leader = studio.members.find(m => m.role === 'Руководитель' || m.role === 'TeamLead' || m.role === 'Заместитель'); // Or whoever sent it, but we only have studioCode here. 
          // Better to pass sender name from client or find by socket.
          // For now, let's just say "Руководство".
